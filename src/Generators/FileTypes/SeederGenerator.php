 ...
 * SeederGenerator
 *
 * Generates seeder files based on the provided settings.
 *
 * @package Proto\Generators\FileTypes
 */
class SeederGenerator extends AbstractFileGenerator
{
	/**
	 * Generates a seeder file.
	 *
	 * @param object $settings The settings for the seeder file generation.
	...
	public function generate(object $settings): bool
	{
		...
		$fileName = $this->getFileName($settings->className . 'Seeder');
		$template = new Templates\SeederTemplate($settings);
		return $this->saveFile($dir, $fileName, $template);
	}

	/**
	 * Returns the full directory path where the seeder file should be saved.
	 *
